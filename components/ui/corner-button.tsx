...
export interface CornerButtonProps
  ...
  accentColor?: string;
  /** Label colour on the accent background. @default "#0008" */
  labelColor?: string;
  ...
  return (
    <div
      ...
      style={
        {
          "--accent": accentColor,
          // color-mix keeps the glow working when the accent is a var() or a
          // named colour, where string concatenation of an alpha suffix breaks.
          "--accent-glow": `color-mix(in srgb, ${accentColor} 33%, transparent)`,
          "--label": labelColor,
    ...
    >
      ...
      <style>{`
        ...
        /* Darken rather than wash to white — a white hover erases any label
           colour that isn't near-black. */
        .corner-btn:hover {
        ...
        /* Children render inside this span, so icons passed as children align
           with the label and inherit its colour. The bundled pencil is a
           sibling of the span and keeps its own .corner-btn-svg styling. */
        .corner-btn-text {
          display: inline-flex;
          align-items: center;
          gap: 0.5rem;
        ...
        /* ── Lines ────────────────────────── */
        /* Decoration only — must never intercept clicks meant for the button. */
        .corner-line {
          position: absolute;
          pointer-events: none;
          transition: all 0.3s ease-in-out;
